...
class MixerSubsystem:
    ...
    def run_command(self, cue: dict):
        message = ""
        match cue["action"]:
            case "enable_channels": # Turns on any Input/Output Channel 
                for channel in cue["channels"]: # ex. command["channels"] = [0, 5, 8]
                    message += f"set MIXER:Current/Channel/{cue["fadertype"]}/On {channel} 0 1 \n "
                self.socket.sendall(message.encode())
            case "disable_channels":# Turns off any Input/Output Channel 
                for channel in cue["channels"]: # ex. command["channels"] = [0, 5, 8]
                    message += f"set MIXER:Current/Channel/{cue["fadertype"]}/On {channel} 0 0 \n "
                self.socket.sendall(message.encode())
            case "set_faders_on_channels":
                for channel, value in cue["channels"].items(): # ex. command["channels"] = {0: -32768, 5: -5000, 8: 0}
                    pass #Todo
